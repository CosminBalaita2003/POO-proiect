...
class Personaj
{ /// clasa care retine numele personajului si actorul care il joaca
    string nume;
    Actor actor;
...
public:
    ...
    {
        out << "Personajul " << p.nume << " este interpretat de " << p.actor.get_prenume() << ' ' << p.actor.get_nume() << " care are " << p.actor.get_varsta() << " ani."
            << "Si are " << p.actor.get_nominalizari() << " Nominalizari" << '\n';
        return out;
    }
};
...
public:
    ...
    friend ostream &operator<<(ostream &out, const Film &f)
    {
        ...
        {
            out << f.p[i].get_nume() << " este interpretat de " << f.p[i].get_actor().get_prenume() << ' ' << f.p[i].get_actor().get_nume() << " care are " << f.p[i].get_actor().get_varsta() << " ani, "
                << " avand " << f.p[i].get_actor().get_nominalizari() << " nominalizari" << '\n';
        }
        return out;
    }
...
int main()
{
    ...
    cout << "bilet 3D\n"
         << b3;
    cout << "- - - -- - - - ";
    cout << "\nPreteul final al biletului simplu este: " << b.get_pret_final() << endl;
